...
class LMSClient:
    ...
    def login(self, username, password):
        return self.stub.Login(lms_pb2.LoginRequest(username=username, password=password))

    def logout(self, token):
        return self.stub.Logout(lms_pb2.LogoutRequest(token=token))

    def post(self, token, post_type, data):
        return self.stub.Post(lms_pb2.PostRequest(token=token, type=post_type, data=data))
